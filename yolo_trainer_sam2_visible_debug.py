...
class MustatilGUI(tk.Tk):
    ...
    def _build_detect_tab(self):
        ...
        ttk.Label(
            preview_frame,
            text="Red = YOLO box fallback. Cyan outline/fill = SAM2 mask polygon. GeoJSON still exports georeferenced polygons when possible.",
            foreground="#555",
        ).pack(anchor="w")
    ...
    def redraw_detection_preview(self):
        ...
        for feat in self.det_preview_features:
            ...
            if bbox and len(bbox) == 4:
                x1, y1, x2, y2 = bbox
                sx1, sy1 = ox + float(x1) * scale, oy + float(y1) * scale
                sx2, sy2 = ox + float(x2) * scale, oy + float(y2) * scale
                color = "cyan" if source == "sam2_mask" else "red"
                self.det_canvas.create_rectangle(sx1, sy1, sx2, sy2, outline=color, width=2)
                label = f"{float(conf):.2f} {source}" if isinstance(conf, (int, float)) else source
        ...
        self.det_canvas.create_text(
            ...
            anchor="nw",
            fill="white",
            text=f"Image: {im.width}x{im.height} | features: {features_count} | SAM2 masks: {sam_shown} | boxes: {bbox_shown}",
        )
    ...
    def detect(self):
        ...
        try:
            import rasterio
            with rasterio.open(img_path) as src_r:
                transform = src_r.transform
                crs_name = src_r.crs.to_string() if src_r.crs else None
        except Exception:
            transform = None
            ...
            for x in range(0, W, stride):
                ...
                if use_sam2 and sam_model is not None and local_dets:
                    for i, det in enumerate(local_dets):
                        try:
                            lb = det["local_bbox"]
                            # Robust path: one bbox prompt per SAM2 call. Slower, but much easier to verify.
                            sam_results = sam_model.predict(arr, bboxes=[lb], verbose=False)
                            polys = sam_masks_to_global_polygons(sam_results, x, y)
                            if polys:
                                # Choose largest polygon by point count; SAM returns one or more masks.
                                best_poly = max(polys, key=len)
                ...
                for i, det in enumerate(local_dets):
                    ...
                    if geom_source == "sam2_mask":
                        # Keep pixel polygon for internal preview alignment.
                ...
                if n % 10 == 0:
                    self.log_detect(
                        ...
                        f"SAM2 masks={sam_success} | fallback boxes={sam_fallback}"
                    )
        ...
        self.log_detect(f"FINAL: YOLO boxes={yolo_detections}; SAM2 masks={sam_success}; fallback boxes={sam_fallback}")
